...
class RFLib_Model_Cache
{
     ...
     */
    public function __construct(RFLib_Model_Abstract $model, $options, $tagged = null)
    {
        $this->_model = $model;
    ...
    */
    ...
    {
        if ('Class' != $frontend) {
            throw new RFLib_Model_Exception('Frontend type must be Class');
        }
    ...
    public function __call($method, $params)
    {
        if (!is_callable(array($this->_model, $method))) {
            throw new RFLib_Model_Exception('Method ' . $method . ' does not exist in class ' . get_class($this->_model) );
        }
        $cache = $this->getCache();
